...
import { notif, deleteNotifiaction, deleteSignUpNot, notifSignup, errorSignInNots, deleteSignInError, login} from "../src/features/loginSlice"
import { auth, provider, googleProvider, signPop } from "../features/firebase"
...
const LoginForm:React.FC<{onAddHandler:(text:string, text2: string) => void}> = (props) => {
    ...
    const signInGoogleHandler = () => {
        ...
        signPop(auth, provider).then((res) => {
            ...
            const credentials = googleProvider.credentialFromError(err);

            console.log("Error credentials",credentials);
        })
    ...
    return (
        <div className="text-center mt-10 flex flex-col justify-center items-center mb-10">
            ...
            {notification && <div className="w-[28%] bg-green-400 p-2 rounded-xl flex justify-between items-center">
            
                <div className="font-bold ml-2">You succesfully logout</div>
                <svg xmlns="http://www.w3.org/2000/svg"  onClick={deleteNotificationHandler} className="h-6 w-6 text-black cursor-pointer hover:scale-125 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                 <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                 </svg>
                </div>}

            {notificationSign && <div className="w-[28%] bg-green-400 p-2 rounded-xl flex justify-between items-center">
            
            <div className="font-bold ml-2">You succesfully created account. Now log in</div>
            <svg xmlns="http://www.w3.org/2000/svg" onClick={deleteUserNotHandler} className="h-6 w-6 text-black cursor-pointer hover:scale-125  mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
             <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
             </svg>
            </div>}
            ...
            <form className="form bg-wheat-400 border-gray shadow-gray-300 bg-white rounded-3xl" onSubmit={submitHandler}>
                ...
                <div className="grid mobile:grid-cols-1 tablet:grid-cols-2 desktop:grid-cols-3 gap-5 mt-7 mb-8 w-4/5">
                    <button type="button" onClick={signInGoogleHandler} className="buttonLogo border-gray w-full">
                    {img1}
